...
BEGIN 
     CASE  
        ...
        WHEN OP=6 THEN C:= 1 / A; --Fracción de A
        WHEN OP=7 THEN C:= 1 / B; --Fracción de B
        ELSE 
        DBMS_OUTPUT.PUT_LINE('UD. NO HA SELECCIONADO UNA OPERACIÓN INCORRECTA');
     END CASE;                              
     RETURN C;
END;
...
SELECT 'HOLA' Mensaje FROM DUAL;
SELECT 10+5 Sumando FROM DUAL;
...
SELECT OPERACION (10, 2, 6)  "Fracción A" FROM DUAL;
SELECT OPERACION (10, 2, 7)  "Fracción B" FROM DUAL;
...
BEGIN
    ...
    Valor := OPERACION(10, 5, 6);
    DBMS_OUTPUT.PUT_LINE('Fracción de A: ' || Valor);
    Valor := OPERACION(10, 15, 7);
    DBMS_OUTPUT.PUT_LINE('Fracción de B: ' || Valor);
END;



-- USO DE TABLAS EN LAS FUNCIONES.
----------------------------------------------------------------------------------------------------------
-- EJEMPLO 2:
----------------------------------------------------------------------------------------------------------
...
-- CUANTO SUMAN LOS PRECIOS DE LOS LIBROS POR CADA AREA.
SELECT * FROM LIBRO;
...
DECLARE
      ...
      TOTAL_IMPORTE NUMBER:=0; IGV NUMBER:=0; TOTAL_PAGAR NUMBER:=0;
BEGIN
...
SET SERVEROUTPUT ON;
DECLARE
  C LIBRO.CODLIB%TYPE:= &Codigo; ---> 1000
...
CREATE OR REPLACE FUNCTION Conteo_Generos
      (Gen Varchar2) ---> 'Masculino','Femenino'
RETURN INT
IS
    V_CONTEO INT:=0;
BEGIN
     -- Esta consulta la hizo pikachu
...
--FORMA 1: USANDO LA TABLA DUAL.
SELECT Conteo_Generos ('Masculino') "VARONES" From Dual;
SELECT Conteo_Generos ('Femenino') "DAMAS" From Dual;
...
DECLARE
     ...
     F_CONTEO INT:=0;
BEGIN
     V_CONTEO:=Conteo_Generos('Masculino');
     F_CONTEO:=Conteo_Generos('Femenino');
     DBMS_OUTPUT.PUT_LINE('===================================');
     DBMS_OUTPUT.PUT_LINE('CONTEO DE DAMAS   : ' || F_CONTEO);
     DBMS_OUTPUT.PUT_LINE('CONTEO DE VARONES : ' || V_CONTEO);
     DBMS_OUTPUT.PUT_LINE('TOTAL CONTEO      : ' || (V_CONTEO+F_CONTEO));
     DBMS_OUTPUT.PUT_LINE('===================================');
END;


----------------------------------------------------------------------------------------------------------
--EJEMPLO 6: Pagina 8 del manual
----------------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------
-- Caso Caleb:
-- profe y si se requiere sumar los libros de Lprog y cad, cómo seria?
SELECT * FROM LIBRO; CREATE OR REPLACE FUNCTION Sumar_Area
       (Area Varchar2) -->'LPROG', 'ANALISIS', 'CAD', 'ARTE','PROYECTOS', 'LIDERAZGO'
RETURN NUMBER
IS
    TOTAL NUMBER := 0;
...
        SELECT SUM(PRECIOLIB) INTO TOTAL FROM LIBRO WHERE AREALIB = Area;
        RETURN TOTAL;
END; -- SOLUCION CALEB - BLOQUE ANÓNIMO SET SERVEROUTPUT ON;
DECLARE 
        SUMAR NUMBER := 0;
BEGIN
        SUMAR:=  Sumar_Area('LPROG') + Sumar_Area('CAD');
        DBMS_OUTPUT.PUT_LINE('LPROG: ' || Sumar_Area('LPROG'));
        DBMS_OUTPUT.PUT_LINE('CAD  : ' || Sumar_Area('CAD'));
        DBMS_OUTPUT.PUT_LINE('LPROG + CAD = ' || SUMAR);
END;
